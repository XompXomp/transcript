...
// Start thirty-first proxy server
const proxy31 = spawn('node', ['stt-proxy-31.js'], {
  stdio: 'inherit',
  cwd: __dirname
});
...
proxy31.on('close', (code) => {
  console.log(`Proxy 31 exited with code ${code}`);
});
...
console.log('Proxy 31 will run on ws://localhost:8060');
